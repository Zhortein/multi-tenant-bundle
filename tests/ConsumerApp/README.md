...
The fixture also compiles a fresh production container while recording named-autowiring deprecations. It verifies that `MigrateTenantsCommand` receives Doctrine's `default` connection both in the single-connection configuration and with explicit `default` and `reporting` connections and EntityManagers, then repeats the check with a custom-named `primary` default connection. DoctrineBundle 3.3 exposes the `default` target used by `#[Target]`, while the bundle's compiler pass detects the legacy DoctrineBundle 2.19 alias and custom-named defaults without branching on dependency versions. This covers the only bundle constructor parameter that matches a framework-generated named alias. The other audited integration arguments are either unqualified type autowiring (Doctrine connections and EntityManagers, loggers and mailers) or explicit service references (configured cache pools, decorated cache services, migration configuration, Messenger factories and transports), so they do not require `#[Target]`.
