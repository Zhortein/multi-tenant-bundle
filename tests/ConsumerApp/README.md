...
docker run --rm -e DATABASE_STRATEGY=shared_db -v /tmp/multi-tenant-consumer:/consumer -w /consumer php:8.3-cli vendor/bin/phpunit --no-coverage
...
docker run --rm -e DATABASE_STRATEGY=multi_db -v /tmp/multi-tenant-consumer:/consumer -w /consumer php:8.3-cli vendor/bin/phpunit --no-coverage
```
